                ...
                <div className="absolute bottom-0 left-0 w-full p-8 pb-10">
                   <div className="flex flex-wrap gap-2 mb-4">
                      {selectedProject.tags.slice(0, 2).map(tag => (
                        <span key={tag} className="px-2 py-1 bg-white/10 backdrop-blur-sm rounded text-[10px] uppercase font-mono tracking-wider">
                          {tag}
                        </span>
                      ))}
                   </div>
                   <h2 className="text-3xl font-medium tracking-tight mb-2">{selectedProject.title}</h2>
                   <p className="text-sm text-[var(--color-paper-dim)] line-clamp-2 leading-relaxed">
                     {selectedProject.tagline}
                   </p>
                </div>
             </div>
           ) : (
             // DEFAULT STATE: Skills & Philosophy
