...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project, index, onClick, onHover }) => {
  ...
  return (
    <a 
      ...
      className="group relative cursor-pointer block text-left touch-manipulation"
      onClick={(e) => {
        e.preventDefault();
        onClick(project.id);
      }}
    ...
    >
      <div className="flex items-center w-full py-5 px-4 
                      ...
                      active:bg-[var(--hover-bg)] active:scale-[0.98]
                      transition-all duration-200 ease-soft relative overflow-hidden">
        
        {/* Col 1: ID - Fixed Width (Kept Mono for technical feel) */}
        <div className="w-12 md:w-16 font-mono text-xs opacity-30 text-[var(--color-ink)] group-hover:opacity-100 group-active:opacity-100 transition-colors">
          {formattedIndex}
        </div>

        {/* Col 2: Title */}
        <div className="flex-1 pr-8 md:pr-32">
          <h3 className="text-xl md:text-4xl font-medium tracking-tight text-[var(--color-ink)] transition-colors group-hover:translate-x-2 group-active:translate-x-2 duration-300">
            {project.title}
          </h3>
        </div>

        {/* Hover Guidance Label - Desktop */}
